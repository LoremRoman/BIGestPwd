...
class LoginSystemNew:
    ...
    def create_login_interface(self, on_success_callback):
        ...
        self.root.title("BIGestPwd 2.3 - Login Seguro")
    ...
    def create_usb_login_interface(self, user_profile):
        ...
        if user_profile:
            tk.Label(center_box, text=f"👤 {user_profile['display_name']}", 
                    font=('Segoe UI', 12, 'bold'), bg=self.widgets.bg_color, fg=self.widgets.accent_color).pack(pady=(0, 20))
        
        # Icono USB
        tk.Label(center_box, text="💾", font=('Segoe UI', 64), bg=self.widgets.bg_color, fg=self.widgets.accent_color).pack()
        tk.Label(center_box, text="Modo USB Bypass", font=('Segoe UI', 18, 'bold'), 
                 bg=self.widgets.bg_color, fg='white').pack(pady=(10, 5))
        
        # Card estado
        status_card = tk.Frame(center_box, bg=self.widgets.card_bg, padx=20, pady=20)
        status_card.pack(fill='x', pady=30, ipadx=20)
        
        tk.Label(status_card, text="Conecta tu llave de seguridad", font=('Segoe UI', 11), 
                 bg=self.widgets.card_bg, fg='white').pack()
        
        self.usb_status = tk.Label(status_card, text="🔍 Buscando...", font=('Segoe UI', 10), 
                                 bg=self.widgets.card_bg, fg=self.widgets.warning_color)
        self.usb_status.pack(pady=(10, 0))
        
        # Botones
        self.usb_login_btn = self.widgets.create_modern_button(
            center_box, "🔓 Entrar con USB", self.attempt_usb_login, self.widgets.success_color, width=25
        )
        self.usb_login_btn.pack(pady=(0, 10))
        self.usb_login_btn.config(state='disabled', bg='#4b5563')
        
        self.widgets.create_modern_button(
            center_box, "🔄 Verificar de nuevo", self.verify_usb_status, self.widgets.accent_color, width=25
        ).pack(pady=(0, 20))
        
        tk.Label(center_box, text="O usa otro método:", bg=self.widgets.bg_color, fg=self.widgets.text_secondary).pack()
        
        self.widgets.create_modern_button(
            center_box, "📱 Usar Código / Contraseña", self.switch_to_totp_mode, '#8b5cf6', width=25
        ).pack(pady=10)
    ...
    def create_totp_login_interface(self, user_profile):
        ...
        if user_profile:
            tk.Label(center_box, text=f"👤 {user_profile['display_name']}", 
                    font=('Segoe UI', 12, 'bold'), bg=self.widgets.bg_color, fg=self.widgets.accent_color).pack(pady=(0, 15))
        
        # Escudo centrado
        tk.Label(center_box, text="🛡️", font=('Segoe UI', 64), bg=self.widgets.bg_color, fg=self.widgets.accent_color).pack()
        tk.Label(center_box, text="Inicio de Sesión", font=('Segoe UI', 22, 'bold'), bg=self.widgets.bg_color, fg='white').pack(pady=(5, 25))
        ...
        # Password
        tk.Label(form_frame, text="Contraseña Maestra", font=('Segoe UI', 10, 'bold'), 
                 bg=self.widgets.bg_color, fg=self.widgets.text_secondary, anchor='w').pack(fill='x')
        ...
        # TOTP
        tk.Label(form_frame, text="Código de Verificación (TOTP)", font=('Segoe UI', 10, 'bold'), 
                 bg=self.widgets.bg_color, fg=self.widgets.text_secondary, anchor='center').pack(fill='x')
        
        self.totp_entry = self.widgets.create_styled_entry(form_frame)
        self.totp_entry.config(font=('Consolas', 16, 'bold'), justify='center', width=10)
        self.totp_entry.pack(pady=(5, 20))
        
        # Botones
        self.widgets.create_modern_button(
            center_box, "🚀 Iniciar Sesión", self.attempt_totp_login, self.widgets.success_color, width=25
        ).pack(pady=5)
        
        if self.usb.is_configured():
            self.widgets.create_modern_button(
                center_box, "💾 Usar Bypass USB", self.switch_to_usb_mode, self.widgets.accent_color, width=25
            ).pack(pady=10)
    ...
    def verify_usb_status(self):
        try:
            if self.usb.verify_device():
                self.usb_status.config(text="✅ USB Autorizado Detectado", fg=self.widgets.success_color)
                self.usb_login_btn.config(state='normal', bg=self.widgets.success_color)
            else:
                self.usb_status.config(text="❌ USB no detectado", fg=self.widgets.danger_color)
                self.usb_login_btn.config(state='disabled', bg='#4b5563')
        except: pass
    ...
    def attempt_usb_login(self):
        if self.usb.verify_device():
            ...
            if pwd:
                if db_manager.verify_master_password(pwd): self.on_success_callback(pwd)
                else: WindowHelper.show_custom_message(self.root, "Error", "Contraseña incorrecta", is_error=True)
        else: self.verify_usb_status()
    ...
    def get_password_for_usb_login(self):
        ...
        tk.Label(main, text="🔐 Confirmar Identidad", font=('Segoe UI', 14, 'bold'), bg=self.widgets.bg_color, fg='white').pack(pady=(0, 15))
        tk.Label(main, text="Ingresa tu contraseña para desbloquear.", bg=self.widgets.bg_color, fg=self.widgets.text_secondary).pack(pady=(0, 20))
        ...
        self.widgets.create_modern_button(btn_f, "Desbloquear", submit, self.widgets.success_color).pack(side='right', padx=5)
        self.widgets.create_modern_button(btn_f, "Cancelar", win.destroy, self.widgets.text_secondary).pack(side='right', padx=5)
    ...
    def attempt_totp_login(self):
        ...
        if not pwd:
            WindowHelper.show_custom_message(self.root, "Error", "Falta contraseña", is_error=True); return
        
        try:
            methods = {'master_password': pwd}
            if code: methods['totp_offline'] = code
            if self.mfa.validate_authentication(methods, pwd): self.on_success_callback(pwd)
            else:
                WindowHelper.show_custom_message(self.root, "Error", "Credenciales inválidas", is_error=True)
    ...
    def show_usb_setup_info(self):
        WindowHelper.show_custom_message(self.root, "Información", "Configura un USB en las opciones.")