...
class PasswordChangeModal:
    ...
    def create_modal(self):
        ...
        try:
            self.modal.iconbitmap("icon.ico")
        except:
            pass
        ...
        tk.Label(
            main_frame,
            text="Esta acción re-encriptará todas tus contraseñas.",
            font=("Segoe UI", 10),
            bg=self.widgets.bg_color,
        ...
        tk.Label(form_frame, text="Actual:", **lbl_style).grid(
            row=0, column=0, pady=10, sticky="w"
        )
        self.current_password_var = tk.StringVar()
        curr_entry = self.widgets.create_styled_entry(
            form_frame, var=self.current_password_var, show="•"
        )
        ...
        self.widgets.create_modern_button(
            form_frame,
            "⌨️",
            lambda: self.virtual_kb.create_keyboard(curr_entry),
            self.widgets.accent_color,
            width=4,
        ).grid(row=0, column=2)

        tk.Frame(form_frame, bg="#333", height=1).grid(
            row=1, column=0, columnspan=3, sticky="ew", pady=15
        )

        tk.Label(form_frame, text="Nueva:", **lbl_style).grid(
            row=2, column=0, pady=10, sticky="w"
        )
        self.new_password_var = tk.StringVar()
        new_entry = self.widgets.create_styled_entry(
            form_frame, var=self.new_password_var, show="•"
        )
        new_entry.grid(row=2, column=1, sticky="ew", ipady=5, padx=(10, 5))
        self.widgets.create_modern_button(
            form_frame,
            "⌨️",
            lambda: self.virtual_kb.create_keyboard(new_entry),
            self.widgets.accent_color,
            width=4,
        ).grid(row=2, column=2)

        tk.Label(form_frame, text="Confirmar:", **lbl_style).grid(
            row=3, column=0, pady=10, sticky="w"
        )
        self.confirm_password_var = tk.StringVar()
        conf_entry = self.widgets.create_styled_entry(
            form_frame, var=self.confirm_password_var, show="•"
        )
        ...
        self.change_btn = self.widgets.create_modern_button(
            btn_frame,
            "🔄 Re-encriptar Todo",
        ...
        self.widgets.create_modern_button(
            btn_frame, "Cancelar", self.modal.destroy, self.widgets.text_secondary
        ).pack(side="right", padx=5)

        self.modal.update_idletasks()
        WindowHelper.center_window(
            self.modal, self.modal.winfo_reqwidth(), self.modal.winfo_reqheight()
        )
    ...
    def validate_password_strength(self, *args):
        ...
        if not password:
            self.strength_label.config(
                text="Fortaleza: --", fg=self.widgets.text_secondary
            )
            return
        strength = 0
        if len(password) >= 8:
        ...
        if strength <= 2:
            self.strength_label.config(
                text="Fortaleza: Débil", fg=self.widgets.danger_color
            )
        elif strength <= 4:
            self.strength_label.config(
                text="Fortaleza: Media", fg=self.widgets.warning_color
            )
        else:
            self.strength_label.config(
                text="Fortaleza: Fuerte", fg=self.widgets.success_color
            )
        self.validate_form()
    ...
    def validate_password_match(self, *args):
        ...
        if new_p and conf_p:
            if new_p == conf_p:
                self.strength_label.config(
                    text="✅ Contraseñas coinciden", fg=self.widgets.success_color
                )
            else:
                self.strength_label.config(
                    text="❌ No coinciden", fg=self.widgets.danger_color
                )
        self.validate_form()
    ...
    def validate_form(self):
        ...
        conf_p = self.confirm_password_var.get()
        valid = (
            bool(curr.strip()) and len(new_p) >= 8 and new_p == conf_p and new_p != curr
        )
        if valid:
            self.change_btn.config(state="normal", bg=self.widgets.success_color)
        else:
            self.change_btn.config(state="disabled", bg="#4b5563")
    ...
    def change_master_password(self):
        ...
        if not db_manager.verify_master_password(current_password):
            WindowHelper.show_custom_message(
                self.modal, "Error", "Contraseña actual incorrecta", is_error=True
            )
            return
        ...
        if messagebox.askyesno(
            "Confirmar Re-encriptación",
            "Esto re-encriptará TODAS tus contraseñas.\n¿Estás seguro?",
            parent=self.modal,
        ):
    ...
    def execute_password_change(self, current_password, new_password):
        ...
        if success:
            ...
            if self.on_success_callback:
                self.on_success_callback(new_password)
            self.modal.destroy()
        else:
            WindowHelper.show_custom_message(
                self.modal, "Error", "Error crítico al cambiar la llave", is_error=True
            )
        ...
        try:
            ...
            with sqlite3.connect(db_manager.db_path, check_same_thread=False) as conn:
                ...
                for entry in entries:
                    ...
                    if entry["notes"]:
                        enc_notes, notes_salt = encryption_system.encrypt_data(
                            entry["notes"], new_password
                        )
...
class USBManagementModal:
    ...
    def create_modal(self):
        ...
        self.usb_list_container.bind(
            "<Configure>", lambda e: canvas.configure(scrollregion=canvas.bbox("all"))
        )
        canvas.create_window(
            (0, 0), window=self.usb_list_container, anchor="nw", width=540
        )
        ...
        self.add_btn = self.widgets.create_modern_button(
            btn_frame, "➕ Añadir USB", self.add_new_usb, self.widgets.success_color
        )
        self.add_btn.pack(side="right")
        self.widgets.create_modern_button(
            btn_frame, "Cerrar", self.modal.destroy, self.widgets.text_secondary
        ).pack(side="right", padx=10)
        self.load_usb_list()
        WindowHelper.center_window(self.modal, 600, 650)

    def load_usb_list(self):
        for widget in self.usb_list_container.winfo_children():
            widget.destroy()
        devices = self.usb.get_authorized_devices()
        if not devices:
            tk.Label(
                self.usb_list_container,
                text="No hay llaves configuradas",
            ...
            for device in devices:
                self.create_usb_card(device)
        if len(devices) >= 2:
            self.add_btn.config(state="disabled", bg="#4b5563")
        else:
            self.add_btn.config(state="normal", bg=self.widgets.success_color)
    ...
    def create_usb_card(self, device):
        ...
        self.widgets.create_modern_button(
            card,
            "🗑️",
            lambda d=device: self.delete_usb_device(d),
            self.widgets.danger_color,
            width=4,
        ).pack(side="right")

    def delete_usb_device(self, device):
        if messagebox.askyesno(
            "Eliminar USB", f"¿Revocar acceso a '{device['name']}'?", parent=self.modal
        ):
            ...
            if success:
                ...
                WindowHelper.show_custom_message(
                    self.modal, "Error", msg, is_error=True
                )

    def add_new_usb(self):
        USBSetupModal(
            self.modal, self.master_password, on_success_callback=self.on_usb_added
        )
...
class TOTPManagementModal:
    ...
    def create_modal(self):
        ...
        status_col = (
            self.widgets.success_color if is_conf else self.widgets.danger_color
        )
        tk.Label(
            main_frame,
            text=status_txt,
        ...
        self.widgets.create_modern_button(
            main_frame, "Cerrar", self.modal.destroy, self.widgets.text_secondary
    ...
    def setup_totp(self):
        ...
        w = MFASetupWizard(
            self.modal, self.master_password, on_complete_callback=on_done
        )
        w.show_step(2)
...
class TOTPQRModal:
    def __init__(self, parent, qr_image, title="Escanea este código"):
        ...
        try:
            ...
            tk.Label(
                main, text="Error imagen", bg=self.widgets.bg_color, fg="red"
            ).pack()

        tk.Label(
            main,
            text="Escanea con Google Authenticator",
            bg=self.widgets.bg_color,
            fg="white",
        ).pack(pady=10)
        self.widgets.create_modern_button(
            main, "Listo", self.modal.destroy, self.widgets.success_color
        ).pack()
...
class USBSetupModal:
    def __init__(self, parent, master_password, on_success_callback=None):
        ...
        self.scan_btn = self.widgets.create_modern_button(
            btn_frame, "🔄 Escanear", self.scan, self.widgets.accent_color
        )
    ...
    def scan(self):
        ...
        try:
            ...
            available = [
                d
                for d in connected
                if self.usb.get_usb_uuid(d["path"]) not in authorized
            ]
            if not available:
                tk.Label(
                    self.list_frame,
                    text="No se encontraron USBs nuevos",
                    bg=self.widgets.card_bg,
                    fg=self.widgets.text_secondary,
                ).pack(pady=20)
            else:
                for dev in available:
                    ...
                    tk.Label(
                        f,
                        text=f"{dev['name']} ({dev['size_gb']}GB)",
                        bg="#2d2d2d",
                        fg="white",
                    ).pack(side="left")
                    self.widgets.create_modern_button(
                        f,
                        "Seleccionar",
                        lambda d=dev: self.configure(d),
                        self.widgets.success_color,
                        width=10,
                    ).pack(side="right")
        except Exception as e:
            tk.Label(
                self.list_frame, text=f"Error: {e}", fg="red", bg=self.widgets.card_bg
            ).pack()
        self.scan_btn.config(text="🔄 Escanear", state="normal")

    def configure(self, device):
        res, msg = self.usb.register_usb_device(
            device["name"], device["path"], self.master_password
        )
        if res:
            if self.on_success_callback:
                self.on_success_callback()
...
class SecurityVerificationModal:
    ...
    def create_modal(self):
        ...
        tk.Label(
            main_frame,
            text=f"Estás a punto de: {self.operation_name}",
            font=("Segoe UI", 11),
            bg=self.widgets.bg_color,
            fg=self.widgets.text_secondary,
            wraplength=350,
        ...
        btn_frame.pack(fill="x", pady=10)
        self.widgets.create_modern_button(
            btn_frame, "Cancelar", self.cancel_operation, self.widgets.text_secondary
        ).pack(side="left", padx=10)
        self.widgets.create_modern_button(
            btn_frame, "✅ Confirmar", self.verify_identity, self.widgets.success_color
        ).pack(side="right", padx=10)
