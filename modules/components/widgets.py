...
class ModernWidgets:
    ...
    def create_modern_button(self, parent, text, command, color, width=None, height=1):
        ...
        if width: btn_config['width'] = width
            
        btn = tk.Button(parent, **btn_config)
        
        def on_enter(e):
            if btn['state'] == 'normal': btn['bg'] = self.adjust_brightness(color, 1.2)
        def on_leave(e):
            if btn['state'] == 'normal': btn['bg'] = color
    ...
    def create_styled_entry(self, parent, var=None, show=None):
        return tk.Entry(
            ...
            insertbackground=self.text_primary, relief='flat', bd=1, show=show
        )
    ...
    def setup_treeview_style(self):
        ...
        style.configure('Treeview', background=self.card_bg, foreground=self.text_primary,
                       fieldbackground=self.card_bg, borderwidth=0, font=('Segoe UI', 10), rowheight=35)
        style.configure('Treeview.Heading', background='#2d2d2d', foreground=self.text_primary,
                       relief='flat', font=('Segoe UI', 9, 'bold'), padding=[5, 10])
        ...
        style.configure("Vertical.TScrollbar", gripcount=0, background="#2d2d2d", darkcolor="#1a1a1a",
            lightcolor="#1a1a1a", troughcolor="#0a0a0a", bordercolor="#1a1a1a", arrowcolor="white")
        style.map("Vertical.TScrollbar", background=[('active', '#404040'), ('disabled', '#1a1a1a')])
        return style