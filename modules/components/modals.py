...
class PasswordGeneratorModal:
    ...
    def create_modal(self):
        ...
        self.security_label = tk.Label(
            header_frame, text="Débil", font=('Segoe UI', 10, 'bold'),
            bg='#0a0a0a', fg='#ef4444'
        )
        self.security_label.pack(side='right')

        # --- RESULTADO ---
        result_frame = tk.Frame(main_container, bg='#1a1a1a', padx=15, pady=15, relief='flat')
        ...
        tk.Button(actions_frame, text="🔄 Regenerar", command=self.generate_password,
                 bg='#3b82f6', fg='white', relief='flat', font=('Segoe UI', 9), cursor='hand2').pack(side='left', padx=5)
        tk.Button(actions_frame, text="📋 Copiar", command=self.copy_password,
                 bg='#8b5cf6', fg='white', relief='flat', font=('Segoe UI', 9), cursor='hand2').pack(side='left', padx=5)

        # --- CONTROLES ---
        controls_frame = tk.LabelFrame(main_container, text="Personalización", bg='#0a0a0a', fg='gray', font=('Segoe UI', 9))
        ...
        tk.Label(len_frame, text="Longitud:", bg='#0a0a0a', fg='white', font=('Segoe UI', 10)).pack(side='left')
        self.length_label = tk.Label(len_frame, text="16", bg='#0a0a0a', fg='#3b82f6', font=('Segoe UI', 10, 'bold'))
        self.length_label.pack(side='right')
        
        self.length_var = tk.IntVar(value=16)
        tk.Scale(
            controls_frame, from_=8, to=64, orient='horizontal', variable=self.length_var,
            command=self.update_length_label, bg='#0a0a0a', fg='white', troughcolor='#333',
        ...
        for i, (icon, text, var) in enumerate(checks):
            ...
            tk.Checkbutton(
                f, text=f"{text} ({icon})", variable=var, command=self.on_options_change,
                bg='#0a0a0a', fg='white', selectcolor='#2563eb', activebackground='#0a0a0a',
    ...
    def calculate_security_level(self, length, selected_options):
        option_count = sum([self.lower_var.get(), self.upper_var.get(), self.digits_var.get(), self.symbols_var.get()])
        security_score = 0
        
        if length >= 20: security_score += 3
        elif length >= 16: security_score += 2
        elif length >= 12: security_score += 1
        
        security_score += option_count
        
        if security_score >= 6: return "maxima", "🔵", "Excelente"
        elif security_score >= 5: return "optima", "🟢", "Segura"
        elif security_score >= 3: return "moderada", "🟡", "Moderada"
        else: return "baja", "🔴", "Débil"
    
    def update_security_indicator(self):
        length = self.length_var.get()
        selected_options = [self.lower_var.get(), self.upper_var.get(), self.digits_var.get(), self.symbols_var.get()]
        level, emoji, text = self.calculate_security_level(length, selected_options)
        
        colors = {"maxima": "#3b82f6", "optima": "#10b981", "moderada": "#f59e0b", "baja": "#ef4444"}
    ...
    def generate_balanced_password(self, length, characters):
        if not characters: return "Selecciona opciones"
        password = []
        char_sets = []
        if self.lower_var.get(): char_sets.append(string.ascii_lowercase)
        if self.upper_var.get(): char_sets.append(string.ascii_uppercase) 
        if self.digits_var.get(): char_sets.append(string.digits)
        if self.symbols_var.get(): char_sets.append(self.symbols)
        
        min_per_type = max(1, length // (len(char_sets) * 2)) if len(char_sets) > 0 else 0
        for char_set in char_sets:
            for _ in range(min_per_type): password.append(secrets.choice(char_set))
    ...
    def generate_password(self):
        ...
        if self.symbols_var.get(): characters += self.symbols
    ...
    def copy_password(self):
        ...
        if password and " " not in password:
            ...
            self.modal.after(500, lambda: self.password_display.config(bg='#1a1a1a', fg='#10b981'))
...
class PasswordEditModal:
    ...
    def create_modal(self):
        ...
        # Header
        tk.Label(
            main_frame, text="✏️ Editar Contraseña", 
            font=('Segoe UI', 18, 'bold'), bg='#0a0a0a', fg='white'
        ...
        entry_style = {'font': ('Segoe UI', 10), 'bg': '#2d2d2d', 'fg': 'white', 
                      'insertbackground': 'white', 'relief': 'flat', 'bd': 1}

        # 1. Categoría (sticky va en el grid)
        tk.Label(form_frame, text="Categoría:", **lbl_style).grid(row=0, column=0, padx=(0, 15), pady=10, sticky='e')
        
        categories = db_manager.get_categories()
        cat_names = [cat['name'] for cat in categories]
        self.category_var = tk.StringVar(value=self.password_data['category'])
        cat_combo = ttk.Combobox(form_frame, textvariable=self.category_var, values=cat_names, state='readonly')
        cat_combo.grid(row=0, column=1, sticky='ew', pady=10)
        
        # 2. Título
        tk.Label(form_frame, text="Título:", **lbl_style).grid(row=1, column=0, padx=(0, 15), pady=10, sticky='e')
        ...
        # 3. Usuario
        tk.Label(form_frame, text="Usuario:", **lbl_style).grid(row=2, column=0, padx=(0, 15), pady=10, sticky='e')
        ...
        tk.Label(form_frame, text="Contraseña:", **lbl_style).grid(row=3, column=0, padx=(0, 15), pady=10, sticky='e')
        ...
        self.widgets.create_modern_button(tools_frame, "👁️", self.toggle_password_visibility, '#f59e0b', width=3).pack(side='left', padx=(0,2))
        self.widgets.create_modern_button(tools_frame, "🎲", self.generate_new_password, '#10b981', width=3).pack(side='left', padx=2)
        self.widgets.create_modern_button(tools_frame, "⌨️", 
            lambda: VirtualKeyboard(self.modal).create_keyboard(self.password_entry), '#3b82f6', width=3
        ).pack(side='left', padx=2)

        # 5. URL
        tk.Label(form_frame, text="URL:", **lbl_style).grid(row=4, column=0, padx=(0, 15), pady=10, sticky='e')
        ...
        # 6. Notas
        tk.Label(form_frame, text="Notas:", **lbl_style).grid(row=5, column=0, padx=(0, 15), pady=10, sticky='ne')
    ...
    def save_changes(self):
        ...
        if not all([cat_name, title, pwd]):
            WindowHelper.show_custom_message(self.modal, "Error", "Faltan campos obligatorios", is_error=True)
            return
        ...
        if not cat_id:
            WindowHelper.show_custom_message(self.modal, "Error", "Categoría inválida", is_error=True)
            return
            
        success = db_manager.update_password_entry(
            self.password_data['id'], cat_id, title, user, pwd, url, notes, self.master_password
        )
        ...
        if success:
            ...
            WindowHelper.show_custom_message(self.modal, "Error", "No se pudo guardar", is_error=True)